...
UINT32 tpm_get_free_session(BYTE type)
{
    ...
    for (i = 0; i < TPM_MAX_SESSIONS; i++) {
        if (read_TPM_STANY_DATA_sessions_type(i) == TPM_ST_INVALID) {
            write_TPM_STANY_DATA_sessions_type(i, type);
...
void tpm_invalidate_sessions(TPM_HANDLE handle)
{
    ...
    int i;
    for (i = 0; i < TPM_MAX_SESSIONS; i++) {
        type = read_TPM_STANY_DATA_sessions_type(i);
        session_handle = read_TPM_STANY_DATA_sessions_handle(i);
        if ((type == TPM_ST_OSAP && session_handle == handle)
            || (type == TPM_ST_DSAP && session_handle == handle)
            || (type == TPM_ST_TRANSPORT && session_handle == handle))
            write_TPM_STANY_DATA_sessions_zero(i);
